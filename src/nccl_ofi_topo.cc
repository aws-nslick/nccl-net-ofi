...
static nccl_ofi_topo_data_vec_t *nccl_ofi_topo_data_vec_create(size_t size)
{
	...
	vec = static_cast<decltype(vec)>(calloc(1, sizeof(nccl_ofi_topo_data_vec_t)));
	if (!vec) {
	...
	if (size > 0) {
		vec->size = size;
		vec->data = static_cast<decltype(vec->data)>(calloc(size, sizeof(nccl_ofi_topo_data_t)));
...
nccl_ofi_topo_t *nccl_ofi_topo_create(struct fi_info *info_list)
{
	...
	nccl_ofi_topo_t *ofi_topo = static_cast<decltype(ofi_topo)>(calloc(1, sizeof(nccl_ofi_topo_t)));
	if (!ofi_topo) {
			       ...
			       const char *prop_name, char *prop)
{
	...
	const std::string path_format = "/sys/bus/pci/devices/%04x:%02x:%02x.%01x/%s"s;
	size_t path_len;
	char *path = NULL;

        if ((path_len = snprintf(NULL, 0, path_format.data(), domain, bus, dev, func, prop_name)) < 0) {
		NCCL_OFI_WARN("Failed to determine device property path length of property %s. ERROR: %s",
			      prop_name, strerror(errno));
		ret = -errno;
		goto error;
	}
	path = static_cast<char *>(malloc(path_len + 1));
	if (!path) {
	...
	/* Create file path */
	if (snprintf(path, path_len + 1, path_format.c_str(), domain, bus, dev, func, prop_name) < 0) {
...
{
	struct hwloc_obj_attr_u::hwloc_pcidev_attr_s attr{};
    using namespace std::string_view_literals;
	/* Override the following PCI width and speed of libfabric NICs with fallback values */
	constexpr auto override_width = "255\0"sv;
	constexpr auto override_speed = "Unknown\0"sv;
	...
	*speed_idx = 0;
	...
	if (is_nic && strncmp(override_speed.data(), prop_str, override_speed.length()) == 0) {
		/* Override speed */
	...
	if (is_nic && strncmp(override_width.data(), prop_str, override_width.length()) == 0) {
		/* Override width */
...
static int write_nic(hwloc_obj_t node, FILE *file, int indent)
{
	...
	struct hwloc_obj_attr_u::hwloc_pcidev_attr_s *pcidev = &node->attr->pcidev;
 ...
 *		Indentation
 */
static int write_pci_opening_tag(FILE *file, struct hwloc_obj_attr_u::hwloc_pcidev_attr_s *pcidev, int indent)
{
	int rc = fprintf(file,
			 "%*s"
			 "<pci "
...
{
	struct hwloc_obj_attr_u::hwloc_pcidev_attr_s *pcidev = &node->attr->bridge.upstream.pci;
