...
static const char* get_platform_type(void)
{
	...
	platform_type = static_cast<decltype(platform_type)>(malloc(sizeof(char)*platform_type_len));
	...
	while ((feof(fd) == 0) && (ferror(fd) == 0) && ((ch = fgetc(fd)) != '\n')) {
		...
		if (len >= platform_type_len) {
			platform_type = static_cast<decltype(platform_type)>(realloc(platform_type, len + platform_type_len));
		}
	}
...
int configure_nvls_option(void)
{
	...
	if (getenv("NCCL_NVLS_ENABLE") == NULL) {
		nccl_get_version = reinterpret_cast<decltype(nccl_get_version)>(dlsym(RTLD_DEFAULT, "ncclGetVersion"));
