...
constexpr ncclResult_t nccl_net_ofi_retval_translate(int const& retval)
{
	/*
...
ncclResult_t nccl_net_ofi_listen(int dev_id, void *handle, void **lComm)
{
	...
	const auto ret = base_ep->listen(base_ep, static_cast<nccl_net_ofi_conn_handle_t*>(handle), listen_comm);
...
ncclResult_t nccl_net_ofi_connect(int dev_id, void *handle, void **sComm)
{
	...
	if (ofi_handle->state.stage == COMM_CREATE_START) {
		...
		const auto ret = base_dev->get_ep(base_dev, &base_ep);
	...
	/* Connect */
	auto ret = base_ep->connect(base_ep, reinterpret_cast<nccl_net_ofi_conn_handle_t*>(handle), reinterpret_cast<nccl_net_ofi_send_comm_t **>(sComm));
	if (ret != 0) {
		base_ep->release_ep(base_ep);
	}
				...
				void **mhandle)
{
	...
	nccl_net_ofi_comm_t *base_comm = reinterpret_cast<decltype(base_comm)>(comm);
    ...
    {
        auto tcomm = reinterpret_cast<nccl_net_ofi_send_comm_t *>(base_comm);
        ret = tcomm->regMr(tcomm, data, size, type, mhandle);
    }
    break;
	case NCCL_NET_OFI_RECV_COMM:
    {
        auto tcomm = reinterpret_cast<nccl_net_ofi_recv_comm_t *>(base_comm);
        ret = tcomm->regMr(tcomm, data, size, type, mhandle);
    }
    break;
	default:
...
ncclResult_t nccl_net_ofi_deregMr(void *comm, void *mhandle)
{
	...
	case NCCL_NET_OFI_SEND_COMM: {
        auto tcomm = reinterpret_cast<nccl_net_ofi_send_comm_t *>(base_comm);
        ret = tcomm->deregMr(tcomm, reinterpret_cast<nccl_net_ofi_mr_handle_t*>(mhandle));
    }
    break;
	case NCCL_NET_OFI_RECV_COMM: {
        auto tcomm = reinterpret_cast<nccl_net_ofi_send_comm_t *>(base_comm);
				      ...
				      int fd, void** mhandle)
{
	...
	case NCCL_NET_OFI_SEND_COMM: {
        auto tcomm = reinterpret_cast<nccl_net_ofi_send_comm_t *>(base_comm);
		ret = tcomm->regMrDmaBuf(tcomm, data, size, type, offset, fd, handle);
    }
		break;
	case NCCL_NET_OFI_RECV_COMM: {
        auto tcomm = reinterpret_cast<nccl_net_ofi_recv_comm_t *>(base_comm);
...
{
	nccl_net_ofi_send_comm_t *send_comm = reinterpret_cast<decltype(send_comm)>(sComm);
