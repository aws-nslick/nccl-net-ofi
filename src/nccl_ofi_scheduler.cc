						...
						int num_rails)
{
	...
	schedule = reinterpret_cast<decltype(schedule)>(nccl_ofi_freelist_entry_alloc(reinterpret_cast<nccl_ofi_freelist_t *>(scheduler_p->schedule_fl)));
					  ...
					  nccl_net_ofi_scheduler_t **scheduler_p)
{
	...
	*scheduler_p = NULL;
	...
	scheduler = static_cast<decltype(scheduler)>(malloc(sizeof(nccl_net_ofi_threshold_scheduler_t)));
	if (!scheduler) {
