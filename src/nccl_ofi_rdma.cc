...
#include "nccl_ofi.h"
#if HAVE_CUDA
#include "nccl_ofi_cuda.hpp"
#endif
#include "nccl_ofi_param.hpp"
#include "nccl_ofi_rdma.hpp"
#include "nccl_ofi_math.hpp"
#include "tracepoint.hpp"
#include "nccl_ofi_scheduler.hpp"
#include "nccl_ofi_topo.hpp"
#include "nccl_ofi_memcheck.hpp"
#include "nccl_ofi_ofiutils.hpp"
				 ...
				 nccl_ofi_properties_t *props)
{
	...
	if (ret == 0) {
		...
		static_assert(NUM_COMM_ID_BITS < 31,
					    ...
					    nccl_net_ofi_rdma_req_t *bounce_req)
{
	...
	nccl_net_ofi_rdma_req_t *req = reinterpret_cast<decltype(req)>(elem);
	rdma_req_send_data_t *send_data = get_send_data(req);
					     ...
					     nccl_net_ofi_rdma_req_t *bounce_req)
{
	...
	nccl_net_ofi_rdma_req_t *recv_req = reinterpret_cast<decltype(recv_req)>(elem);
	rdma_req_recv_data_t *recv_data = get_recv_data(recv_req);
	...
	(nccl_net_ofi_rdma_ep_t *ep, uint64_t data)
{
	...
	return reinterpret_cast<nccl_net_ofi_rdma_req_t*>(elem);
}
				      ...
				      nccl_net_ofi_ep_rail_t *rail)
{
	...
	for (comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		req = reinterpret_cast<decltype(req)>(cq_entry[comp_idx].op_context);
		...
		if (comp_flags & FI_SEND) {
			...
			if (!(comp_flags & FI_REMOTE_CQ_DATA)) {
				...
				ret = handle_bounce_recv(static_cast<nccl_ofi_rdma_msg_type_t>(*msg_type), ep, rail->rail_id, &cq_entry[comp_idx], req);
					 ...
					 nccl_net_ofi_ep_rail_t *rail)
{
	...
	if (err_entry.flags & FI_REMOTE_WRITE) {
		...
		req = reinterpret_cast<decltype(req)>(err_entry.op_context);
	}
...
static inline void zero_nccl_ofi_req(nccl_net_ofi_rdma_req_t *req)
{
	...
	req->type = NCCL_OFI_RDMA_TYPE_NONE;
}
							...
							nccl_net_ofi_ep_rail_t *rail)
{
	...
	nccl_net_ofi_rdma_bounce_fl_item_t *bounce_fl_item =
		reinterpret_cast<decltype(bounce_fl_item)>(nccl_ofi_freelist_entry_alloc(reinterpret_cast<nccl_ofi_freelist_t*>(ep->bounce_buff_fl)));
	if (!bounce_fl_item) {
...
{
	return static_cast<nccl_net_ofi_rdma_mr_handle_t *>(calloc(1, sizeof(nccl_net_ofi_rdma_mr_handle_t)
...
{
	nccl_net_ofi_rdma_ep_t *ep = reinterpret_cast<decltype(ep)>(ep_void_ptr);
	...
	freelist_regmr_fn_handle_t *freelist_handle = static_cast<decltype(freelist_handle)>(malloc(sizeof(freelist_regmr_fn_handle_t)));
	if (!freelist_handle) {
...
{
	freelist_regmr_fn_handle_t *freelist_handle = reinterpret_cast<decltype(freelist_handle)>(handle);
...
static inline nccl_net_ofi_rdma_req_t *allocate_req(nccl_ofi_freelist_t *fl)
{
	...
	nccl_net_ofi_rdma_req_t *req = reinterpret_cast<decltype(req)>(nccl_ofi_freelist_entry_alloc(fl));
	if (OFI_UNLIKELY(req == NULL)) {
				...
				nccl_net_ofi_rdma_req_t *recv_req)
{
	...
	nccl_net_ofi_rdma_ctrl_fl_item_t *ctrl_fl_item =
		reinterpret_cast<decltype(ctrl_fl_item)>(nccl_ofi_freelist_entry_alloc(reinterpret_cast<nccl_ofi_freelist_t*>(r_comm->ctrl_buff_fl)));
			 ...
			 nccl_net_ofi_req_t **base_req)
{
    ...
    nccl_net_ofi_rdma_req_t *bounce_req = NULL;
    rdma_req_bounce_data_t *bounce_data = NULL;
	...
	ep = reinterpret_cast<decltype(ep)>(r_comm->base.base.ep);
	assert(ep != NULL);

	device = reinterpret_cast<decltype(device)>(ep->base.device);
	assert(device != NULL);
	...
	if (eager) {
		bounce_req = static_cast<decltype(bounce_req)>(elem);
		bounce_data = get_bounce_data(bounce_req);
		if (bounce_data->recv_len == 0) {
...
static int recv_close(nccl_net_ofi_recv_comm_t *recv_comm)
{
	...
	device = reinterpret_cast<decltype(device)>(base_ep->device);
...
{
	return static_cast<nccl_net_ofi_rdma_recv_comm_t *>(calloc(1, sizeof(nccl_net_ofi_rdma_recv_comm_t)
							...
							nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	comm_id = nccl_ofi_idpool_allocate_id(ep->comm_idpool);
			     ...
			     nccl_net_ofi_listen_comm_t **listen_comm)
{
	...
	/* Build listen_comm */
	l_comm = static_cast<decltype(l_comm)>(calloc(1, sizeof(nccl_net_ofi_rdma_listen_comm_t)));
			   ...
			   nccl_net_ofi_xfer_info_t *xfer_info)
{
	...
	ssize_t rc;
	/* Post RDMA write */
	rc = fi_writedata(comm_rail->local_ep, reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(send_data->buff) + reinterpret_cast<uintptr_t>(xfer_info->offset)),
				...
				nccl_net_ofi_xfer_info_t *xfer_info)
{
	...
	ssize_t rc;
	/* Post eager send */
	rc = fi_senddata(comm_rail->local_ep,
                     reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(send_data->buff) + reinterpret_cast<uintptr_t>(xfer_info->offset)),
                     xfer_info->msg_size, desc, send_data->wdata, comm_rail->remote_addr, req);
			      ...
			      nccl_net_ofi_ep_rail_t *ep_rail)
{
	...
	freelist_regmr_fn_handle_t *fl_mr_handle = static_cast<decltype(fl_mr_handle)>(bounce_fl_item->fl_reginfo.mr_handle);
...
static int post_rdma_ctrl(nccl_net_ofi_rdma_req_t *req)
{
	...
	nccl_net_ofi_rdma_ctrl_fl_item_t *ctrl_fl_item = reinterpret_cast<decltype(ctrl_fl_item)>(send_ctrl_data->ctrl_fl_item);

	/* Unpack mr_handle */
	freelist_regmr_fn_handle_t * fl_handle = reinterpret_cast<decltype(fl_handle)>(ctrl_fl_item->fl_reginfo.mr_handle);
	nccl_net_ofi_rdma_mr_handle_t *mr_handle = reinterpret_cast<decltype(mr_handle)>(fl_handle->mr_handle);
...
static int post_eager_copy(nccl_net_ofi_rdma_req_t *req)
{
	...
	/* Unpack mr_handle */
	freelist_regmr_fn_handle_t * fl_handle = reinterpret_cast<decltype(fl_handle)>(bounce_data->bounce_fl_item->fl_reginfo.mr_handle);
	nccl_net_ofi_rdma_mr_handle_t *bounce_mr_handle = reinterpret_cast<decltype(bounce_mr_handle)>(fl_handle->mr_handle);

	nccl_net_ofi_rdma_mr_handle_t *dest_mr_handle = reinterpret_cast<decltype(dest_mr_handle)>(recv_data->dest_mr_handle);
			 ...
			 nccl_net_ofi_mr_handle_t *mhandle, nccl_net_ofi_req_t **base_req)
{
	...
	if (have_ctrl) {
		 ...
		 */
		nccl_net_ofi_rdma_req_t *bounce_req = static_cast<decltype(bounce_req)>(elem);
		copy_ctrl_data(bounce_req, req);
...
static int send_close(nccl_net_ofi_rdma_send_comm_t *s_comm)
{
	...
	ep = reinterpret_cast<decltype(ep)>(s_comm->base.base.ep);
...
static int blocked_send_close(nccl_net_ofi_send_comm_t *send_comm)
{
	...
	s_comm = reinterpret_cast<decltype(s_comm)>(send_comm);

	/* Validate endpoint */
	ep = reinterpret_cast<nccl_net_ofi_rdma_ep_t *>(s_comm->base.base.ep);
	if (OFI_UNLIKELY(ep == NULL)) {
		NCCL_OFI_WARN("Invalid endpoint provided");
		return -EINVAL;
	}
...
{
	return static_cast<nccl_net_ofi_rdma_send_comm_t *>(calloc(1, sizeof(nccl_net_ofi_rdma_send_comm_t)
				    ...
				    nccl_net_ofi_ep_t **base_ep)
{
	...
	ep = static_cast<nccl_net_ofi_rdma_ep_t*>(pthread_getspecific(device->ep_key));
	if (!ep) {
		int num_rails = device->num_rails;

		/* Allocate endpoint */
		ep = static_cast<decltype(ep)>(calloc(1, sizeof(nccl_net_ofi_rdma_ep_t)));
		if (!ep) {
			ret = -ENOMEM;
	...
	if (ep->ref_cnt == 0) {
		ep->rails = static_cast<decltype(ep->rails)>(calloc(ep->num_rails, sizeof(nccl_net_ofi_ep_rail_t)));
		if (!ep->rails) {
			ret = -ENOMEM;
		...
		ep->comms = static_cast<decltype(ep->comms)>(calloc(NCCL_OFI_RDMA_MAX_COMMS, sizeof(nccl_net_ofi_comm_t*)));
		if (!ep->comms) {
		...
		ep->comm_idpool = static_cast<decltype(ep->comm_idpool)>(malloc(sizeof(nccl_ofi_idpool_t)));
								 ...
								 int num_infos)
{
	...
	nccl_net_ofi_rdma_device_rail_t *device_rails =
		static_cast<decltype(device_rails)>(calloc(num_infos, sizeof(nccl_net_ofi_rdma_device_rail_t)));
			   ...
			   nccl_net_ofi_plugin_t **plugin_p)
{
	...
	plugin = static_cast<decltype(plugin)>(malloc(sizeof(nccl_net_ofi_plugin_t)));
	if (!plugin) {
	...
	base_devs = static_cast<decltype(base_devs)>(calloc(num_devs, sizeof(nccl_net_ofi_rdma_device_t *)));
	if (!base_devs) {
	...
	for (dev_id = 0 ; dev_id != num_devs ; ++dev_id) {
		...
		/* Allocate device */
		device = static_cast<decltype(device)>(calloc(1, sizeof(nccl_net_ofi_rdma_device_t)));
		if (!device) {
