...
nccl_ofi_msgbuff_t *nccl_ofi_msgbuff_init(uint16_t max_inprogress, uint16_t bit_width)
{
	...
	msgbuff = static_cast<nccl_ofi_msgbuff_t*>(malloc(sizeof(nccl_ofi_msgbuff_t)));
	if (!msgbuff) {
		NCCL_OFI_WARN("Memory allocation (msgbuff) failed");
		goto error;
	}

    msgbuff->buff = static_cast<nccl_ofi_msgbuff_elem_t *>(malloc(sizeof(nccl_ofi_msgbuff_elem_t)*max_inprogress));
	if (!msgbuff->buff) {
