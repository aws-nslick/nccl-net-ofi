...
#include "nccl_ofi_sendrecv.hpp"
#include "nccl_ofi_freelist.hpp"
...
static inline void zero_nccl_ofi_req(nccl_net_ofi_sendrecv_req_t *req)
{
	...
	req->direction = NCCL_OFI_SENDRECV_NODIR;
}

/*
 * @brief	Prepares sendrecv request for reuse
 */
template <bool dec_inflight_reqs>
                           ...
                           nccl_net_ofi_sendrecv_req_t *req)
{
    ...
    if constexpr (dec_inflight_reqs) {
        (*num_inflight_reqs)--;
    }
						       ...
						       nccl_net_ofi_sendrecv_req_t *req)
{
	...
	return free_req<dec_inflight_reqs>(num_inflight_reqs, nccl_ofi_reqs_fl, dev_id, req);
}
						  ...
						  nccl_net_ofi_sendrecv_req_t *req)
{
	if (req->direction == NCCL_OFI_SENDRECV_SEND) {
		nccl_net_ofi_sendrecv_send_comm_t *s_comm =
			reinterpret_cast<decltype(s_comm)>(base_comm);
		return free_req_send_comm<dec_inflight_reqs>(s_comm, dev_id, req);
	}
	else if (req->direction == NCCL_OFI_SENDRECV_RECV) {
		nccl_net_ofi_sendrecv_recv_comm_t *r_comm =
			reinterpret_cast<decltype(r_comm)>(base_comm);
		return free_req_recv_comm<dec_inflight_reqs>(r_comm, dev_id, req);
	}
	else {
...
static int test(nccl_net_ofi_req_t *base_req, int *done, int *size)
{
    ...
    nccl_net_ofi_sendrecv_ep_t *ep = nullptr;
	...
	ep = reinterpret_cast<decltype(ep)>(base_comm->ep);
	...
	if (OFI_LIKELY(req->state == NCCL_OFI_SENDRECV_REQ_COMPLETED ||
		...
		*done = 1;
		...
		free_req_comm<true>(base_comm, dev_id, req);
	}
	else {
		*done = 0;
	}
...
static nccl_net_ofi_sendrecv_req_t *prepare_recv_req(nccl_net_ofi_sendrecv_listen_comm_t *l_comm)
{
	...
	req = static_cast<decltype(req)>(calloc(1, sizeof(nccl_net_ofi_sendrecv_req_t)));
				  ...
				  nccl_net_ofi_req_t **base_req)
{
 ...
 error:
	if (req)
		free_req_recv_comm<false>(r_comm, dev_id, req);
 exit:
	return ret;
}
							    ...
							    char *remote_ep_addr)
{
	...
	/* Build recv_comm */
	r_comm = static_cast<decltype(r_comm)>(calloc(1, sizeof(nccl_net_ofi_sendrecv_recv_comm_t)));
	if (r_comm == NULL) {
			   ...
			   nccl_net_ofi_recv_comm_t **recv_comm)
{
	...
	*recv_comm = NULL;
	...
	case COMM_RECV_CONN:
		...
		if (conn_info == NULL) {
			conn_info = static_cast<decltype(conn_info)>(calloc(1, sizeof(nccl_ofi_connection_info_t)));
		}
...
static inline char *get_local_address(struct fid_ep *ep)
{
	...
	char *local_ep_addr = static_cast<decltype(local_ep_addr)>(calloc(namelen, sizeof(char)));
			     ...
			     nccl_net_ofi_listen_comm_t **listen_comm)
{
	...
	/* Build listen_comm */
	l_comm = static_cast<decltype(l_comm)>(calloc(1, sizeof(nccl_net_ofi_sendrecv_listen_comm_t)));
				  ...
				  nccl_net_ofi_mr_handle_t *mhandle, nccl_net_ofi_req_t **base_req)
{
	...
	if (OFI_UNLIKELY(s_comm->conn_info && (s_comm->conn_info->connect_to_self == 1))) {
		...
		nccl_net_ofi_sendrecv_req_t *req = reinterpret_cast<decltype(req)>(conn_info->req);

		if (req->state == NCCL_OFI_SENDRECV_REQ_COMPLETED) {
			free_req_send_comm<false>(s_comm, dev_id, req);
			free(conn_info);
					    ...
					    nccl_net_ofi_sendrecv_send_comm_t **s_comm)
{
	...
	*s_comm = NULL;
	...
	ret_s_comm = (nccl_net_ofi_sendrecv_send_comm_t *)
		static_cast<decltype(ret_s_comm)>(calloc(1, sizeof(nccl_net_ofi_sendrecv_send_comm_t)));
	...
	ret_s_comm->conn_info = static_cast<decltype(ret_s_comm->conn_info)>(calloc(1, sizeof(nccl_ofi_connection_info_t)));
				    ...
				    nccl_net_ofi_ep_t **base_ep)
{
	...
	ep = static_cast<nccl_net_ofi_sendrecv_ep_t *>(pthread_getspecific(device->ep_key));
	if (!ep) {
		/* Allocate endpoint */
		ep = static_cast<decltype(ep)>(calloc(1, sizeof(nccl_net_ofi_sendrecv_ep_t)));
		if (!ep) {
			ret = -ENOMEM;
			NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET,
...
found:
	...
	base_devs = static_cast<decltype(base_devs)>(malloc(num_providers * sizeof(nccl_net_ofi_sendrecv_device_t *)));
	if (!base_devs) {
	...
	while (dev_id != num_providers) {
		...
		/* Allocate device */
		nccl_net_ofi_sendrecv_device_t *device = static_cast<decltype(device)>(malloc(sizeof(nccl_net_ofi_sendrecv_device_t)));
		if (!device) {
