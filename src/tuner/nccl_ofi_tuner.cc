...
ncclResult_t nccl_ofi_tuner_init(size_t nRanks, size_t nNodes, ncclDebugLogger_t logFunction)
{
	...
	struct nccl_ofi_tuner_model_params params = {
		.net_lat = static_cast<float>(ofi_nccl_tuner_net_latency()),
	...
	if (!nccl_ofi_tuner_ctx) {
        nccl_ofi_tuner_ctx = static_cast<decltype(nccl_ofi_tuner_ctx)>(calloc(1, sizeof(struct nccl_ofi_tuner_context)));
		if (!nccl_ofi_tuner_ctx) {
