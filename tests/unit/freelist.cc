...
int main(int argc, char *argv[])
{
	...
	for (i = 0 ; i < 8 ; i++) {
		...
		last_buff = reinterpret_cast<decltype(last_buff)>(entry);
	}
	...
	for (i = 0 ; i < 8 ; i++) {
		struct random_freelisted_item *item = reinterpret_cast<decltype(item)>(nccl_ofi_freelist_entry_alloc(freelist));
		if (!item) {
			NCCL_OFI_WARN("allocation unexpectedly failed");
			exit(1);
		}
