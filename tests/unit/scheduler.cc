...
{
	nccl_net_ofi_schedule_t *schedule = reinterpret_cast<decltype(schedule)>(malloc(sizeof(nccl_net_ofi_schedule_t) + num_rails * sizeof(nccl_net_ofi_xfer_info_t)));
	if (!schedule) {
...
int test_multiplexing_schedule()
{
	...
	nccl_net_ofi_schedule_t *ref_schedule = reinterpret_cast<decltype(ref_schedule)>(malloc(sizeof(nccl_net_ofi_schedule_t)
